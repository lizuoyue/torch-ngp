...
class NeRFRenderer(nn.Module):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            if self.opt.use_self_sigma:
                import frnn
                query = xyzs[..., [2, 0, 1]] / self.bound / 2.0 * self.encoder.aabb_size
                pts1 = query.unsqueeze(0)
                pts2 = (self.encoder.pts_sparse.C[:, 1:] / self.encoder.scale).unsqueeze(0)
                frnn_dists, frnn_idxs, frnn_nn, frnn_grid = frnn.frnn_grid_points(
                    ...
                    K=1, r=1.0, grid=None, return_nn=False, return_sorted=True,
                )
                occ = torch.exp(-0.01 * torch.exp(300 * frnn_dists[0, :, 0] - 5))
                sigmas = -torch.log(1.0 - occ) / deltas[:, 0]
            ...
            if self.opt.visualize_point_cloud is not None:
                ...
                import frnn
                query = xyzs[..., [2, 0, 1]] / self.bound / 2.0 * self.encoder.aabb_size
                with open(f'{self.opt.workspace}/{self.opt.visualize_point_cloud}/{self.encoder.vis_count:04d}_spl_pts.txt', 'w') as f:
                    for (x, y, z), (r, g, b) in zip(query.cpu().numpy(), (rgbs.detach().cpu().numpy() * 255.0).astype(np.uint8)):
                        f.write('%.3lf %.3lf %.3lf %d %d %d\n' % (x, y, z, r, g, b))
                occ = 1 - torch.exp(-sigmas.detach() * deltas[:, 0])
                occ_colored = np.clip(occ.cpu().numpy(), 0.0, 1.0)
                occ_colored = plt.get_cmap("viridis")(occ_colored)
                occ_colored = (occ_colored * 255.0).astype(np.uint8)
                with open(f'{self.opt.workspace}/{self.opt.visualize_point_cloud}/{self.encoder.vis_count:04d}_spl_pts_occ.txt', 'w') as f:
                    for (x, y, z), (r, g, b, _) in zip(query.cpu().numpy(), occ_colored):
                        f.write('%.3lf %.3lf %.3lf %d %d %d\n' % (x, y, z, r, g, b))
                ...
                vis_mask = frnn_dists[0, :, 0] > 0

                xxx = np.linspace(0.0, 1.0, 1000)
                yyy = np.exp(-0.01 * np.exp(300 * xxx - 5))
                plt.plot(frnn_dists[0, vis_mask, 0].cpu().numpy(), occ[vis_mask].cpu().numpy(), "o", markersize=1, alpha=0.05)
                plt.plot(xxx, yyy, "-")
                plt.savefig(f"{self.opt.workspace}/{self.opt.visualize_point_cloud}/{self.encoder.vis_count:04d}.png")
                plt.clf()
                
                self.encoder.vis_count += 1
                input(f"\ncheck grid_size {self.grid_size} aabb_size {self.opt.aabb_size}")
        ...
        for xs in X:
            for ys in Y:
                for zs in Z:
                    ...
                    # cascading
                    for cas in range(self.cascade):
                        bound = min(2 ** cas, self.bound) # 1 and 2
                        half_grid_size = bound / self.grid_size # 1/128 and 2/128
                        # scale to current cascade's resolution
                        cas_xyzs = xyzs * (bound - half_grid_size) # [-127/128, 127/128] and [-127/64, 127/64]
                        # add noise in [-hgs, hgs]
                        # cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size
                        # query density
                        n_voxel = round(self.bound / half_grid_size) # 256 and 128
                        ...
                        # if self.opt.visualize_point_cloud:
